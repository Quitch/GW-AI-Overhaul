...
describe("steps under jQuery 2's Deferred", () => {
  ...
  it("without it, a step's throw leaves the fail handler unrun", () => {
  ...
  it("rejects with a step's synchronous throw and skips the rest", () => {
    const start = $.Deferred();
    const ran = [];
    let reason;
    ...
    assert.equal(reason.message, "second broke");
  });
