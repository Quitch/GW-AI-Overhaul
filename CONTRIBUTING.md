...
GitHub Actions runs the same checks automatically on every push to `develop` or `master`, every pull request, and every published release; pushes to `develop` and pull requests also get the SonarCloud coverage gate (see "Test coverage and new code" below). `stylelint.config.mjs` is a Chromium 151 profile: `.browserslistrc` plus `stylelint-no-unsupported-browser-features` checks each declaration against caniuse, guarding against features newer than the engine, and hand-written rules ban the syntax no Blink release ever shipped, which caniuse cannot flag. Don't remove an entry or "fix" the usage it covers as a drive-by: `format:css` runs `stylelint --fix` across the repo, so a mis-set rule there produces CSS the engine silently drops.
