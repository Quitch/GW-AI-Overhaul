define(["coui://ui/mods/com.pa.quitch.gwaioverhaul/shared/loadout_ids.js"], (
  gwoLoadoutIds
) => ({
  setupGwoCards: function (gwoSettings) {
    const loadouts = gwoLoadoutIds.all;
    const basicCards = [
      "gwc_add_card_slot",
    ...
    if (
      ...
      return _.uniq(model.gwoCards.concat(loadouts, basicCards, expandedCards));
    }
  ...
  setupGwoDeck: function (cards, deck, cardsRemaining, promise) {
    _.forEach(model.gwoCards, (cardId, index) => {
  ...
  dealCard: function (params, loaded, loadedCards) {
    ...
    loaded.then(() => {
      const card = _.find(loadedCards, { id: params.id });
