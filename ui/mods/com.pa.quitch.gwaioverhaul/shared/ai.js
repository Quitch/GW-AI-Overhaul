define([
...
], (refereeAIPaths, gwoDifficulty, subcommanderTech, races, brainTable) => {
  ...
  const originSystem = (game) => {
    const galaxy = game.galaxy();
    return galaxy.stars()[galaxy.origin()].system();
  };

  const originSettings = (game) => originSystem(game).gwaio;

  const currentStarAi = (game) =>
    game.galaxy().stars()[game.currentStar()].ai();
  ...
  const warBrain = (alignment, race) => {
    const gwoSettings = originSettings(model.game());
    if (gwoSettings) {
  ...
  const aiInUse = (alignment, race) =>
    races.brainFor(warBrain(alignment, race), race);
  ...
  const factionIndex = (ai) =>
    Array.isArray(ai.faction) ? Number.parseInt(ai.faction[0]) : ai.faction;
  ...
  // See galaxy.md, "Difficulty".
  const bossCommanders = (ai, gwoSettings) => {
    const settings = gwoSettings || {};
    const tier = warTier(settings);
    if (
      ai.boss === true &&
      tier &&
  ...
  const bountyValue = (ai) => {
    const tier = warTier(originSettings(model.game()));
  ...
  return {
    ...
    aiTags: function (ai) {
      return _.times(ai.foes ? 1 + ai.foes.length : 1, (n) => `.ai${n}`);
    },
    ...
    getAIPathSource: function (type, race) {
      ...
      const currentAiInUse = aiInUse(type, race);
    ...
    getAIPathDestination: function (type, options) {
      ...
      const currentAiInUse = aiInUse(type, settings.race);
    ...
    penchantTags: function (penchantName) {
      const penchant = _.find(PENCHANTS, { name: penchantName });
    ...
    aiEconRateWithFloor: function (aiEconRate) {
      ...
      const gwoSettings = _.defaults({}, originSettings(model.game()), {
