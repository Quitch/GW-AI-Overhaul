...
define(["coui://ui/mods/com.pa.quitch.gwaioverhaul/shared/decks.js"], (
  decks,
) => {
  ...
  const registerAll = () => {
    model.gwoDecks = Array.isArray(model.gwoDecks) ? model.gwoDecks : [];

    _.forEach(model.gwoDecks, (descriptor) => {
      if (seen.includes(descriptor)) {
        return;
      }
      ...
      try {
        if (descriptor && decks.byId(descriptor.id)) {
          console.warn(
            `gwoDeckMods: deck ${descriptor.id} registered twice; the later registration wins`,
          );
        }
