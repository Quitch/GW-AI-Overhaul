define({
  difficulties: [
    {
      difficultyName: "!LOC:Beginner",
      customDifficulty: false,
      goForKill: "false",
      microType: 0,
      mandatoryMinions: 0,
      minionMod: 0.13,
      priority_scout_metal_spots: "false",
      factory_build_delay_min: 0,
      factory_build_delay_max: 12,
      unable_to_expand_delay: 0,
      enable_commander_danger_responses: "false",
      per_expansion_delay: 0,
      personality_tags: ["SlowerExpansion"],
      econBase: 0.35,
      econRatePerDist: 0.05,
      max_basic_fabbers: 5,
      max_advanced_fabbers: 5,
      ffa_chance: 25,
      bossCommanders: 1,
      landAnywhereChance: 20,
      suddenDeathChance: 10,
      bountyModeChance: 25,
      bountyModeValue: 0.5,
      factionTechHandicap: 2.5,
      alliedCommanderChance: 100,
      eradicationModeChance: 20,
    },
    {
      difficultyName: "!LOC:Casual",
      customDifficulty: false,
      goForKill: "false",
      microType: 0,
      mandatoryMinions: 0,
      minionMod: 0.13,
      priority_scout_metal_spots: "false",
      factory_build_delay_min: 0,
      factory_build_delay_max: 12,
      unable_to_expand_delay: 0,
      enable_commander_danger_responses: "false",
      per_expansion_delay: 0,
      personality_tags: ["SlowerExpansion"],
      econBase: 0.35,
      econRatePerDist: 0.05,
      max_basic_fabbers: 5,
      max_advanced_fabbers: 5,
      ffa_chance: 25,
      bossCommanders: 1,
      landAnywhereChance: 20,
      suddenDeathChance: 10,
      bountyModeChance: 25,
      bountyModeValue: 0.5,
      factionTechHandicap: 2.5,
      alliedCommanderChance: 20,
      eradicationModeChance: 20,
    },
    {
      difficultyName: "!LOC:Iron",
      customDifficulty: false,
      goForKill: "false",
      microType: 1,
      mandatoryMinions: 0,
      minionMod: 0.16,
      priority_scout_metal_spots: "true",
      factory_build_delay_min: 0,
      factory_build_delay_max: 0,
      unable_to_expand_delay: 0,
      enable_commander_danger_responses: "true",
      per_expansion_delay: 0,
      personality_tags: ["SlowerExpansion"],
      econBase: 0.425,
      econRatePerDist: 0.075,
      max_basic_fabbers: 6,
      max_advanced_fabbers: 6,
      ffa_chance: 25,
      bossCommanders: 1,
      landAnywhereChance: 20,
      suddenDeathChance: 10,
      bountyModeChance: 25,
      bountyModeValue: 0.4,
      factionTechHandicap: 2,
      starting_location_evaluation_radius: 100,
      alliedCommanderChance: 20,
      eradicationModeChance: 15,
    },
    {
      difficultyName: "!LOC:Bronze",
      customDifficulty: false,
      goForKill: "true",
      microType: 2,
      mandatoryMinions: 0,
      minionMod: 0.19,
      priority_scout_metal_spots: "true",
      factory_build_delay_min: 0,
      factory_build_delay_max: 0,
      unable_to_expand_delay: 0,
      enable_commander_danger_responses: "true",
      per_expansion_delay: 0,
      personality_tags: [],
      econBase: 0.5,
      econRatePerDist: 0.1,
      max_basic_fabbers: 7,
      max_advanced_fabbers: 7,
      ffa_chance: 25,
      bossCommanders: 2,
      landAnywhereChance: 20,
      suddenDeathChance: 10,
      bountyModeChance: 25,
      bountyModeValue: 0.4,
      factionTechHandicap: 1.5,
      starting_location_evaluation_radius: 150,
      alliedCommanderChance: 15,
      eradicationModeChance: 15,
    },
    {
      difficultyName: "!LOC:Silver",
      customDifficulty: false,
      goForKill: "true",
      microType: 2,
      mandatoryMinions: 0,
      minionMod: 0.24,
      priority_scout_metal_spots: "true",
      factory_build_delay_min: 0,
      factory_build_delay_max: 0,
      unable_to_expand_delay: 0,
      enable_commander_danger_responses: "true",
      per_expansion_delay: 0,
      personality_tags: [],
      econBase: 0.6,
      econRatePerDist: 0.1,
      max_basic_fabbers: 8,
      max_advanced_fabbers: 8,
      ffa_chance: 25,
      bossCommanders: 2,
      landAnywhereChance: 20,
      suddenDeathChance: 10,
      bountyModeChance: 25,
      bountyModeValue: 0.3,
      factionTechHandicap: 1,
      starting_location_evaluation_radius: 200,
      alliedCommanderChance: 15,
      eradicationModeChance: 10,
    },
    {
      difficultyName: "!LOC:Gold",
      customDifficulty: false,
      goForKill: "true",
      microType: 2,
      mandatoryMinions: 0,
      minionMod: 0.28,
      priority_scout_metal_spots: "true",
      factory_build_delay_min: 0,
      factory_build_delay_max: 0,
      unable_to_expand_delay: 0,
      enable_commander_danger_responses: "true",
      per_expansion_delay: 0,
      personality_tags: [],
      econBase: 0.7,
      econRatePerDist: 0.15,
      max_basic_fabbers: 9,
      max_advanced_fabbers: 9,
      ffa_chance: 25,
      bossCommanders: 3,
      landAnywhereChance: 20,
      suddenDeathChance: 10,
      bountyModeChance: 25,
      bountyModeValue: 0.3,
      factionTechHandicap: 0.5,
      starting_location_evaluation_radius: 250,
      alliedCommanderChance: 10,
      eradicationModeChance: 10,
    },
    {
      difficultyName: "!LOC:Platinum",
      customDifficulty: false,
      goForKill: "true",
      microType: 2,
      mandatoryMinions: 0,
      minionMod: 0.39,
      priority_scout_metal_spots: "true",
      factory_build_delay_min: 0,
      factory_build_delay_max: 0,
      unable_to_expand_delay: 0,
      enable_commander_danger_responses: "true",
      per_expansion_delay: 0,
      personality_tags: ["PreventsWaste"],
      econBase: 1,
      econRatePerDist: 0.15,
      max_basic_fabbers: 10,
      max_advanced_fabbers: 10,
      ffa_chance: 25,
      bossCommanders: 3,
      landAnywhereChance: 20,
      suddenDeathChance: 10,
      bountyModeChance: 25,
      bountyModeValue: 0.2,
      factionTechHandicap: 0,
      starting_location_evaluation_radius: 300,
      alliedCommanderChance: 10,
      eradicationModeChance: 10,
    },
    {
      difficultyName: "!LOC:Diamond",
      customDifficulty: false,
      goForKill: "true",
      microType: 2,
      mandatoryMinions: 0,
      minionMod: 0.49,
      priority_scout_metal_spots: "true",
      factory_build_delay_min: 0,
      factory_build_delay_max: 0,
      unable_to_expand_delay: 0,
      enable_commander_danger_responses: "true",
      per_expansion_delay: 0,
      personality_tags: ["PreventsWaste"],
      econBase: 1.2,
      econRatePerDist: 0.2,
      max_basic_fabbers: 11,
      max_advanced_fabbers: 11,
      ffa_chance: 25,
      bossCommanders: 4,
      landAnywhereChance: 20,
      suddenDeathChance: 10,
      bountyModeChance: 25,
      bountyModeValue: 0.2,
      factionTechHandicap: -0.5,
      starting_location_evaluation_radius: 400,
      alliedCommanderChance: 5,
      eradicationModeChance: 10,
    },
    {
      difficultyName: "!LOC:Uber",
      customDifficulty: false,
      goForKill: "true",
      microType: 2,
      mandatoryMinions: 0,
      minionMod: 0.49,
      priority_scout_metal_spots: "true",
      factory_build_delay_min: 0,
      factory_build_delay_max: 0,
      unable_to_expand_delay: 0,
      enable_commander_danger_responses: "true",
      per_expansion_delay: 0,
      personality_tags: ["PreventsWaste"],
      econBase: 10,
      econRatePerDist: 0,
      max_basic_fabbers: 11,
      max_advanced_fabbers: 11,
      ffa_chance: 25,
      bossCommanders: 4,
      landAnywhereChance: 20,
      suddenDeathChance: 10,
      bountyModeChance: 25,
      bountyModeValue: 0.1,
      factionTechHandicap: -0.5,
      starting_location_evaluation_radius: 400,
      alliedCommanderChance: 5,
      eradicationModeChance: 10,
    },
    {
      difficultyName: "!LOC:Custom",
      customDifficulty: true,
    },
  ],
  // gwoDifficultySettings observable -> the tier field it takes when a preset
  // tier is picked, in the order gw_start/ui.js writes them.
  tierSettings: [
    { name: "goForKill", key: "goForKill" },
    { name: "microType", key: "microType" },
    { name: "mandatoryMinions", key: "mandatoryMinions" },
    { name: "minionMod", key: "minionMod" },
    { name: "priorityScoutMetalSpots", key: "priority_scout_metal_spots" },
    { name: "factoryBuildDelayMin", key: "factory_build_delay_min" },
    { name: "factoryBuildDelayMax", key: "factory_build_delay_max" },
    { name: "unableToExpandDelay", key: "unable_to_expand_delay" },
    {
      name: "enableCommanderDangerResponses",
      key: "enable_commander_danger_responses",
    },
    { name: "perExpansionDelay", key: "per_expansion_delay" },
    { name: "econBase", key: "econBase" },
    { name: "econRatePerDist", key: "econRatePerDist" },
    { name: "maxBasicFabbers", key: "max_basic_fabbers" },
    { name: "maxAdvancedFabbers", key: "max_advanced_fabbers" },
    { name: "ffaChance", key: "ffa_chance" },
    { name: "bossCommanders", key: "bossCommanders" },
    {
      name: "startingLocationEvaluationRadius",
      key: "starting_location_evaluation_radius",
    },
    { name: "landAnywhereChance", key: "landAnywhereChance" },
    { name: "suddenDeathChance", key: "suddenDeathChance" },
    { name: "bountyModeChance", key: "bountyModeChance" },
    { name: "bountyModeValue", key: "bountyModeValue" },
    { name: "factionTechHandicap", key: "factionTechHandicap" },
    { name: "alliedCommanderChance", key: "alliedCommanderChance" },
    { name: "personalityTags", key: "personality_tags" },
    { name: "eradicationModeChance", key: "eradicationModeChance" },
  ],
});
