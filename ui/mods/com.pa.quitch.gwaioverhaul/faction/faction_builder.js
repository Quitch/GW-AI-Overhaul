...
define(["coui://ui/mods/com.pa.quitch.gwaioverhaul/shared/ai_personality.js"], (
  gwoPersonality,
) => {
  ...
  const fromBaseline = (baseline, modifiers) => {
    const built = _.merge(_.cloneDeep(baseline), modifiers);
    const personalityId = gwoPersonality.idOf(modifiers.personality);
    if (personalityId) {
  ...
  const build = (data) => {
    ...
    const randomFrom = data.minions.slice();
    const minions = data.minions.concat([data.randomAI]);
    ...
    return {
      ...
      minions: _.map(minions, (personalityModifiers) =>
        fromBaseline(baseline, personalityModifiers),
      ),
      gwaioRandomSpec: {
        baseline,
