define(function () {
  // These values are the same across all factions.
  // They're overridden by difficulty levels but are used by subcommanders.
  var microType = 0;
  var goForTheKill = false;
  var neuralDataMod = 1;
  var personality_tags = [
    "Default",
    "GWAlly",
    "SlowerExpansion",
    "queller",
    "GalacticWar",
  ];
  var priorityScoutMetalSpots = false;
  var factoryBuildDelayMin = 0;
  var factoryBuildDelayMax = 0;
  var unableToExpandDelay = 0;
  var enableCommanderDangerResponses = false;
  var perExpansionDelay = 0;

  // The faction personalities, in the key order the AI contract lists them;
  // each faction overrides a handful of these in place.
  var factionBaseline = {
    percent_vehicle: 0.65,
    percent_bot: 0.1,
    percent_air: 0.15,
    percent_naval: 0.05,
    percent_orbital: 0.05,
    metal_drain_check: 0.54,
    energy_drain_check: 0.65,
    metal_demand_check: 0.71,
    energy_demand_check: 0.8,
    micro_type: microType,
    go_for_the_kill: goForTheKill,
    neural_data_mod: neuralDataMod,
    personality_tags: personality_tags,
    adv_eco_mod: 1,
    adv_eco_mod_alone: 0.85,
    priority_scout_metal_spots: priorityScoutMetalSpots,
    factory_build_delay_min: factoryBuildDelayMin,
    factory_build_delay_max: factoryBuildDelayMax,
    unable_to_expand_delay: unableToExpandDelay,
    enable_commander_danger_responses: enableCommanderDangerResponses,
    per_expansion_delay: perExpansionDelay,
    fabber_to_factory_ratio_basic: 1,
    fabber_to_factory_ratio_advanced: 1,
    fabber_alone_on_planet_mod: 2,
    basic_to_advanced_factory_ratio: 0,
    factory_alone_on_planet_mod: 0.5,
    min_basic_fabbers: 2,
    max_basic_fabbers: 4,
    min_advanced_fabbers: 3,
    max_advanced_fabbers: 3,
    works_with_queller: true,
  };

  var faction = function (overrides) {
    return _.assign({}, factionBaseline, overrides);
  };

  return {
    legonisMachina: faction({}),
    legonisMachinaBoss: {
      percent_naval: 0,
      percent_orbital: 0.1,
      fabber_to_factory_ratio_basic: 2,
      fabber_to_factory_ratio_advanced: 2,
      fabber_alone_on_planet_mod: 3,
      min_basic_fabbers: 4,
      min_advanced_fabbers: 1,
    },
    legonisMachinaOriginal: {
      percent_vehicle: 0.275,
      percent_bot: 0.275,
      percent_air: 0.35,
      percent_naval: 0.05,
      percent_orbital: 0.05,
      metal_drain_check: 0.75,
      energy_drain_check: 0.85,
      metal_demand_check: 0.75,
      energy_demand_check: 0.85,
      fabber_to_factory_ratio_basic: 1.5,
      min_basic_fabbers: 1,
    },
    armour: {
      percent_vehicle: 1,
      percent_bot: 0,
      percent_air: 0,
      percent_naval: 0,
      percent_orbital: 0,
      works_with_queller: false,
    },
    roboticist: {
      percent_vehicle: 0,
      percent_bot: 1,
      percent_air: 0,
      percent_naval: 0,
      percent_orbital: 0,
      works_with_queller: false,
    },

    foundation: faction({
      percent_vehicle: 0.2,
      percent_bot: 0.05,
      percent_air: 0.25,
      percent_naval: 0.45,
      adv_eco_mod: 1.3,
      fabber_to_factory_ratio_advanced: 2,
    }),
    foundationBoss: {
      adv_eco_mod: 1,
      fabber_to_factory_ratio_basic: 2,
      fabber_to_factory_ratio_advanced: 2,
      fabber_alone_on_planet_mod: 3,
      min_basic_fabbers: 4,
      min_advanced_fabbers: 1,
    },
    foundationOriginal: {
      percent_vehicle: 0.025,
      percent_bot: 0.025,
      percent_air: 0.55,
      percent_naval: 0.35,
      percent_orbital: 0.05,
      metal_drain_check: 0.75,
      energy_drain_check: 0.85,
      metal_demand_check: 0.75,
      energy_demand_check: 0.85,
      fabber_to_factory_ratio_basic: 1.5,
      min_basic_fabbers: 1,
    },
    airForce: {
      percent_vehicle: 0,
      percent_bot: 0,
      percent_air: 1,
      percent_naval: 0,
      percent_orbital: 0,
      works_with_queller: false,
    },
    navy: {
      percent_vehicle: 0,
      percent_bot: 0,
      percent_air: 0,
      percent_naval: 1,
      percent_orbital: 0,
      works_with_queller: false,
    },

    synchronous: faction({
      percent_vehicle: 0.1,
      percent_bot: 0.65,
      adv_eco_mod: 1.3,
      fabber_to_factory_ratio_basic: 1.5,
    }),
    synchronousBoss: {
      percent_naval: 0,
      percent_orbital: 0.1,
      adv_eco_mod: 1,
      fabber_to_factory_ratio_basic: 2,
      fabber_to_factory_ratio_advanced: 2,
      fabber_alone_on_planet_mod: 3,
      min_basic_fabbers: 4,
      min_advanced_fabbers: 1,
    },
    synchronousOriginal: {
      percent_vehicle: 0.15,
      percent_bot: 0.15,
      percent_air: 0.3,
      percent_naval: 0.05,
      percent_orbital: 0.35,
      min_basic_fabbers: 1,
    },
    grunt: {
      percent_vehicle: 0.4,
      percent_bot: 0.3,
      percent_air: 0.2,
      percent_naval: 0.05,
      percent_orbital: 0.05,
      works_with_queller: false,
    },
    dragoon: {
      percent_vehicle: 0,
      percent_bot: 0.2,
      percent_air: 0.7,
      percent_naval: 0.05,
      percent_orbital: 0.05,
      works_with_queller: false,
    },

    revenants: faction({
      percent_vehicle: 0.25,
      percent_bot: 0.2,
      percent_air: 0.05,
      percent_orbital: 0.45,
      adv_eco_mod: 1.3,
      fabber_alone_on_planet_mod: 3,
      factory_alone_on_planet_mod: 0.25,
    }),
    revenantsBoss: {
      adv_eco_mod: 1,
      fabber_to_factory_ratio_basic: 2,
      fabber_to_factory_ratio_advanced: 2,
      fabber_alone_on_planet_mod: 3,
      min_basic_fabbers: 4,
      min_advanced_fabbers: 1,
    },
    revenantsOriginal: {
      percent_vehicle: 0.075,
      percent_bot: 0.075,
      percent_air: 0.15,
      percent_naval: 0.1,
      percent_orbital: 0.6,
      fabber_to_factory_ratio_basic: 1.5,
      min_basic_fabbers: 1,
    },
    spaceInvader: {
      percent_vehicle: 0,
      percent_bot: 0,
      percent_air: 0,
      percent_naval: 0,
      percent_orbital: 1,
      works_with_queller: false,
    },
    astronaut: {
      percent_vehicle: 0,
      percent_bot: 0,
      percent_air: 0.5,
      percent_naval: 0,
      percent_orbital: 0.5,
      works_with_queller: false,
    },

    cluster: faction({
      percent_vehicle: 0.45,
      percent_bot: 0.4,
      percent_air: 0.05,
      adv_eco_mod: 1.5,
      min_basic_fabbers: 3,
      min_advanced_fabbers: 1,
    }),
    clusterBoss: {
      adv_eco_mod: 2,
      fabber_to_factory_ratio_basic: 1.5,
      fabber_alone_on_planet_mod: 3,
      min_basic_fabbers: 4,
    },

    // Generic
    uber: {
      energy_drain_check: 0.72,
      metal_demand_check: 0.8,
      fabber_to_factory_ratio_advanced: 2,
      fabber_alone_on_planet_mod: 3,
      min_basic_fabbers: 3,
      min_advanced_fabbers: 1,
    },
    fabber: {
      fabber_to_factory_ratio_basic: 2,
      fabber_to_factory_ratio_advanced: 2,
      factory_alone_on_planet_mod: 0.25,
      works_with_queller: false,
    },
    defender: {
      metal_drain_check: 0.71,
      energy_drain_check: 0.8,
      metal_demand_check: 0.54,
      energy_demand_check: 0.65,
      adv_eco_mod: 1,
      min_basic_fabbers: 4,
    },
    luddite: {
      adv_eco_mod: 3,
      adv_eco_mod_alone: 2,
    },
    technologist: {
      adv_eco_mod: 0.5,
      adv_eco_mod_alone: 0.5,
      fabber_to_factory_ratio_basic: 3,
      min_basic_fabbers: 4,
      min_advanced_fabbers: 1,
    },
    cautious: {
      neural_data_mod: 0.75,
      min_basic_fabbers: 4,
    },
    aggressive: {
      neural_data_mod: 2,
      min_basic_fabbers: 2,
      min_advanced_fabbers: 1,
    },
    rush: {
      neural_data_mod: 1.5,
      adv_eco_mod: 2,
      min_basic_fabbers: 2,
      min_advanced_fabbers: 1,
    },
    turtle: {
      neural_data_mod: 0.5,
      adv_eco_mod: 0.5,
      adv_eco_mod_alone: 0.5,
      fabber_to_factory_ratio_basic: 3,
      fabber_to_factory_ratio_advanced: 3,
      min_basic_fabbers: 4,
      max_basic_fabbers: 8,
      max_advanced_fabbers: 6,
    },
    absurd: {
      adv_eco_mod: 1.3,
      min_basic_fabbers: 2,
      min_advanced_fabbers: 3,
    },
    factory: {
      fabber_to_factory_ratio_basic: 0.5,
      fabber_to_factory_ratio_advanced: 0.5,
      metal_demand_check: 0.99,
      energy_demand_check: 0.99,
    },
    swarm: {
      basic_to_advanced_factory_ratio: 10,
      fabber_to_factory_ratio_advanced: 3,
      works_with_queller: false,
    },
    economist: {
      metal_drain_check: 0.71,
      energy_drain_check: 0.8,
      metal_demand_check: 0.99,
      energy_demand_check: 0.99,
      adv_eco_mod: 1,
      min_basic_fabbers: 4,
    },
  };
});
