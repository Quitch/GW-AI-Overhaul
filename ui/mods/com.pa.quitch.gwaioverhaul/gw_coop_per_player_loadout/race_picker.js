...
(() => {
  try {
    const raceSelectId = "#gwo-viewer-race-select";
    ...
    model.gwoViewerRacesAvailable = ko.computed(
      () => model.gwoViewerRaceOptions().length > 1,
    );
    ...
    requireGW(
      ...
      (hostWar, raceMods, races, pickerOptions) => {
        ...
        hostWar.load().then((host) => {
          ...
          const startingRace = _.map(host.races, "id").includes(host.race)
            ? host.race
            : races.MLA_ID;

          model.gwoViewerRaceOptions(
            _.map(host.races, (race) => ({
              id: race.id,
              name: loc(race.name),
            })),
          );
          ...
          raceMods.mountRoot().always(() => {
            ...
            CommanderUtility.afterCommandersLoaded(() => {
              const stock = model.commanders();
