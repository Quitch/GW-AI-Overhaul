...
(() => {
  try {
    ...
    const savedRace = settings.playerRace();
    const raceSelectId = "#gwo-race-select";
    ...
    model.gwoRacesAvailable = ko.computed(
      () => model.gwoRaceOptions().length > 1,
    );
    ...
    model.gwoPerPlayerRaceSwitchText = ko.computed(() =>
      model.gwoDraftPerPlayerRace() && model.draftNewGamePerPlayerTechCards()
        ? loc("!LOC:ON")
        : loc("!LOC:OFF"),
    );
    model.toggleGwoDraftPerPlayerRace = () => {
      if (!model.draftNewGamePerPlayerTechCards()) {
        return;
      }
      model.gwoDraftPerPlayerRace(!model.gwoDraftPerPlayerRace());
    };
    model.draftNewGamePerPlayerTechCards.subscribe((value) => {
      if (!value) {
    ...
    // "Function hijacking".
    const openCoopSettingsModal = model.openCoopSettingsModal;
    ...
    const applyCoopSettingsModal = model.applyCoopSettingsModal;
    ...
    requireGW(
      ...
      (raceMods, races, pickerOptions) => {
        ...
        ko.computed(() => {
          const race = races.byId(settings.playerRace());
          const choices = pickerOptions.commanderChoices(
            race,
            model.commanders(),
            races.MLA_ID,
          );
          ...
          if (
            choices.length &&
            !choices.includes(model.selectedCommander.peek())
          ) {
        ...
        ko.computed(() => {
          const cluster = model.playerFaction().name === "Cluster";
        ...
        raceMods.installedRaces().then((info) => {
          const installed = _.map(info.races, "id");

          model.gwoRaceInfo(info);
          model.gwoRaceOptions(
            _.map(info.races, (race) => ({
              id: race.id,
              name: loc(race.name),
            })),
          );
          ...
          const wanted = model.gwoRaceSelectDisabled()
            ? races.MLA_ID
            : savedRace;
          settings.playerRace(
            installed.includes(wanted) ? wanted : races.MLA_ID,
          );
