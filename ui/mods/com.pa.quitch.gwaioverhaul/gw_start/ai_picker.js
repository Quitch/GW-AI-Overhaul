...
(() => {
  try {
    ...
    const buildRows = () => {
      ...
      const raceOptions = model.gwoRaceOptions();
      const namesById = {};
      _.forEach(raceOptions, (option) => {
        namesById[option.id] = option.name;
      });

      const rows = brainTable.rowsFor(
        settings.aiByRace(),
        _.map(raceOptions, "id"),
        settings.ai(),
        settings.aiAlly(),
      ...
      return _.map(rows, (row) => {
        const descriptor = races.byId(row.id);
        return {
          id: row.id,
          stale: row.stale,
    ...
    model.applyGwoAiModal = () => {
      ...
      const stored = _.cloneDeep(settings.aiByRace()) || {};
      _.forEach(model.gwoAiTableRows(), (row) => {
        if (row.stale) {
          return;
        }
    ...
    requireGW(
      ...
      (racesModule, brainTableModule) => {
        ...
        model.gwoRaceOptions.subscribe(() => {
          model.gwoAiTableRows(buildRows());
        });
      },
    );
