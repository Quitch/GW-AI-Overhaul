...
define([
  ...
  const buildGraph = function () {
    ...
    const outerEdges = this.graph.getOuterEdges();
    const innerEdges = _.filter(
      allEdges,
      (testEdge) =>
        !_.some(
          outerEdges,
          (o) => testEdge[0] === o[0] && testEdge[1] === o[1],
        ),
    );
  ...
  const build = function (config) {
    ...
    const rng = config.gwoRng || gwoRng.create(config.seed || 0);
    ...
    _.forEach(builder.stars, (star, index) => {
    ...
    let center = _.reduce(builder.stars, (total, element) => [
      total[0] + element[0],
      total[1] + element[1],
    ]);
    center = [
    ...
    const deltas = _.map(builder.stars, (element) => {
      let delta = [element[0] - center[0], element[1] - center[1]];
    ...
    builder.stars = _.map(builder.stars, (element, index) => {
    ...
    const jitterRng = rng.stream("jitter");
    self.stars(
      _.map(builder.stars, (star) => {
    ...
    _.forEach(self.stars(), (star, index) => {
    ...
    let maxDist = 0;
    builder.reducedGraph.calcDistance(self.origin(), (s, distance) => {
    ...
    const StarSystemTemplates = gwoSystemTemplates.chooseFor(
      chooseStarSystemTemplates,
      config.content,
      config.useEasierSystemTemplate,
    );

    const brackets = config.gwoSystemBrackets;
    ...
    const systemSizeFor = (star, index) => {
      let systemSize;
    ...
    const placeSystem = (star, starSystem) =>
      starSystem.then((system) => {
    ...
    const systemByStar = [];
    if (brackets) {
      const selector = gwoSystemBrackets.selectorFor(
        brackets,
        rng.stream("brackets"),
        config.gwoBiomeProviders,
      );
      const byDistance = _.sortBy(
        _.map(self.stars(), (star, index) => ({
          star,
          index,
        })),
        (entry) => entry.star.distance(),
      );
      _.forEach(byDistance, (entry) => {
    ...
    const starGenerators = _.map(self.stars(), (star, index) => {
      if (brackets) {
    ...
    return $.when.apply($, starGenerators).then(() => self);
  };
