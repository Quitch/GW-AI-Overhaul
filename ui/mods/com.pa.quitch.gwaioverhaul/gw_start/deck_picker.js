...
(() => {
  try {
    ...
    const savedDeck = settings.techCardDeck();
    const deckSelectId = "#game-cards";
    ...
    requireGW(
      ...
      (deckMods, decks, pickerOptions) => {
        ...
        if (all.length > 2) {
          ...
          _.forEach(all, (deck) => {
            if (deck.tooltip) {
              extra += `<BR>${_.escape(loc(deck.name)).toUpperCase()}: ${loc(deck.tooltip)}`;
            }
          });
        ...
        const restored = decks.byId(savedDeck);
