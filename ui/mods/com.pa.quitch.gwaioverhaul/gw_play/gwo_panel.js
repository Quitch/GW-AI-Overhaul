(() => {
  ...
  function gwoWarInfoPanel(gwoSettings) {
    try {
      ...
      const options = (optionsList, setting, text) => {
        if (setting) {
      ...
      requireGW(
        ...
        (
          ...
          const warDeckId = model.gwoSettings.techCardDeck;
          const warDeck = gwoDecks.byId(warDeckId);
          if (warDeck) {
            model.gwoDeck = loc(warDeck.name);
          } else if (warDeckId) {
            model.gwoDeck = `${deckName("Expanded")} (${loc("!LOC:missing:")} ${warDeckId})`;
          }
          ...
          const recordedRaces = gwoSettings.races || {};
          const warRaceIds = _.uniq(
            _.filter(
              _.map(
          ...
          const raceName = (id) => {
            const descriptor = gwoRaces.byId(id);
            return descriptor ? loc(descriptor.name) : id;
          };
          const brainSummary = (side) => {
            const entries = _.map(warRaceIds, (id) => ({
              id,
            ...
            const brains = _.uniq(_.map(entries, "brain"));
            ...
            return _.map(
              entries,
              (entry) => `${raceName(entry.id)}: ${entry.brain}`,
            ).join(", ");
          };
          ...
          api.mods.getMounted("client").then((mods) => {
            ...
            const incompatibleModNames = _.sortBy(
              _.map(incompatibleModsInUse, (incompatibleMod) => {
                const index = _.findIndex(mods, {
                  identifier: incompatibleMod,
                });
          ...
          const raceIcon = (race) => {
            const descriptor = gwoRaces.byId(race) || gwoRaces.byId(playerRace);
          ...
          const updateCoopCommander = (client, human) => {
            const cacheKey = gwoRefereeCoop.clientKey(client.id, client.name);
          ...
          model.gwoPlayer = ko.computed(() => {
            ...
            if (coopCampaign) {
              commanders = _.map(connectedClients, (client) => {
                const cacheKey = gwoRefereeCoop.clientKey(
                  client.id,
                  client.name,
                );
  ...
  let gwoPanelLoaderInitialized = false;
  let gwoPanelLoaderNeedsDispose = false;
  let gwoPanelLoadWarned = false;
