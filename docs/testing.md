...
`scripts/lib/fake-jquery.js` covers exactly the `$`/`api` subset the shipped
co-op code and the deliberately-still-jQuery deal/cards subsystem use.
`scripts/lib/fake-fetch.js` is the counterpart for `shared/gwo_fetch.js`. In
both, a request for a URL with no configured resolver rejects. A test's fixtures
