...
| `game/galactic_war/shared/js/gw_factions.js`              | Adds the Cluster faction.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `game/galactic_war/shared/js/gw_inventory.js`             | Adds the `aiMods` observable that the whole AI-mod pipeline depends on. Changes `removeUnits` to remove _every_ copy of a unit. `addUnits` and `removeUnits` flatten a group nested in the list and ignore a whole `gwoUnit` race table. Deletes the per-pass `setTag` override beside `getTag`'s, and makes `getTag` and `setTag` call `tags.valueHasMutated()` so tag readers update. Suspends loadout banking while a co-op viewer applies the host's inventory ([`coop.md`](coop.md)). Installs GWO's `defeatTeam` from the same `patch` hijack ([`architecture.md`](architecture.md), "Returning from a battle"). Drops stock's `cards.subscribe(applyCards)`, which double-applied - every stock mutator (`gw_game.js` `winTurn`/`load`, `gw_play.js`'s discard splices, `gw_start.js`'s start card) already calls `applyCards`. The extra pass in `gw_start` marked the start card processed before `gw_play` ran its buffs. |
